...
Declare @JobIdCount INT = (SELECT COUNT(1) from @useJobIdFiltering)
...
(
	...
	(
		(@JobIdCount = 0)
			OR
		(EE.jobid IN (SELECT JobId FROM @LastestJobIds))
	)
)
